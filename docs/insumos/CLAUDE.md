...
- **Errores tipados.** `ResultadoAcceso` con motivo enumerado: `VIGENCIA_EXPIRADA`, `AFORO_SUPERADO`, `LISTA_NEGRA`, `ZONA_NO_AUTORIZADA`, `FUERA_DE_PATRON`, `SIN_CONSENTIMIENTO`, `PLACA_DESCONOCIDA`, `CONFIANZA_INSUFICIENTE`, `FALLO_TECNICO`. Prohibido lanzar strings o devolver `null` como señal de negocio.
