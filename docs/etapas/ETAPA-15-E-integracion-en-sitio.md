...
| _(docs)_   | `docs(etapa-15e)`: guía `INTEGRACION_HIKVISION.md`, README, ADR-022, ESTADO, contradicciones, cliente Dart regenerado (A9) |
...
- **En memoria todavía**: alertas, bitácora de órdenes, bloqueos y auditoría de
  seguridad. Un reinicio los pierde; el arranque no lo avisa para estos cuatro.
...
- **La app del residente no muestra aún el enlace de consentimiento** que la
  API devuelve (`enlaceDeConsentimiento`): hoy se copia desde la consola.
...
1. **Aplicar la migración 0035** (`supabase db push`).
