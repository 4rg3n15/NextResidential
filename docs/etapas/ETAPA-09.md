...
### El interruptor `MFA_OBLIGATORIO` — RETIRADO el 2026-09-10

Esta sección describía un interruptor temporal que apagaba la exigencia de `aal2` para los roles administrativos. **Ya no existe**, y no quedó con valor por defecto: se fueron la variable, la rama del guard, el token `POLITICA_MFA` con su fichero, el aviso de arranque, el cartel de la consola, la rama de la ruta de sesión y los dos `.env.example`.

**Por qué se puso, y por qué fue un error de diagnóstico y no una concesión.** Se introdujo porque el cliente no podía entrar y había que desbloquearle. El bloqueo era otro: `SUPABASE_JWKS_URL` apuntaba a `/auth/v1/jwks`, que devuelve `404`, así que la API no obtenía ninguna clave pública y rechazaba **todos** los tokens (D-60). El interruptor no arreglaba nada —tapaba el síntoma en el único camino donde el cliente miraba— y a cambio dejaba RN-20 debilitada y un mensaje de error que apuntaba a una causa inexistente.

**Lo que se conserva de su suite, y por qué.** Un control de seguridad tiene dos modos de fallo, y el peligroso no es el obvio: que **encendido no encienda** no lo descubre nadie, porque nadie prueba lo que da por hecho. Retirado el interruptor, ese modo es el único que queda. Las pruebas que lo vigilaban siguen en pie, renombradas, y se les añadió el control anti-regresión en tres capas —API, consola y camino del navegador—: los tres levantan los procesos **con** `MFA_OBLIGATORIO=false` en el entorno y exigen que el segundo factor se siga pidiendo.
...
| **DT-12** | **Configuración externa sin verificar — la familia «dos suites que se solapan y dejan un intervalo».** El diagnóstico era correcto y la ronda del 2026-09-10 encontró **cuatro apariciones más**: la URL del JWKS (D-60, la decimocuarta, con el doble construido con la forma del error), el JWKS que responde vacío (D-60b), los atributos `style` de los gráficos (D-63, jsdom no aplica CSP y el navegador visitaba el tablero sin datos) y `/ready` publicando una cadena fija por PostgreSQL (D-64). **Cerrada parcialmente**: hay comprobación de arranque contra el recurso real para JWKS, PostgreSQL, bucket de evidencia y recuperación. Quedan **FCM** —doble, y la ETAPA 11 es su dueña— y **Supabase Realtime frente al canal SSE propio**, que sigue siendo lo que se mide | Reducida a dos                                                                                                                                              |
