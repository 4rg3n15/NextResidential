...
| **D-134** | La autorización SEMBRADA con placa (`ABC9999`) vale ocho horas desde la siembra: la prueba «4 bis» de D-25 se ponía roja sola con la base levantada desde la víspera. **Lo destapó la primera corrida del verificador**, que salió FALLIDA por ella                                                                                                                                                                             | **Corregido**: la prueba crea su propia autorización vigente                                                    |
| **D-135** | El bloque del guion de sitio en `apps/api/.env.example` declaraba `*_HOST/_CANAL` de terminal y videoportero **sin lector conocido** para `entorno-declarado.mjs`, y el control nuevo `frontera-extensibilidad.mjs` (O2) **no estaba en la base del trinquete de ramas**. **Lo destapó la segunda corrida del verificador** (paso 9)                                                                                            | **Corregido**: una variable por línea, exentas con su lector; base del trinquete actualizada con `--actualizar` |
| **D-136** | La prueba de borrado definitivo de vehículo pasaba **claims vacíos** a `RepositorioPadronPg` y salía verde sólo porque el dueño de la función `SECURITY DEFINER` era el superusuario local. Tras el paso 12 (`--modo-supabase`, dueño sin `BYPASSRLS`) la RLS forzada respondía «el vehículo no existe» — lo mismo que respondería Supabase. **Lo destapó el paso 14 (estabilidad) de la segunda corrida**, tres veces seguidas | **Corregido**: la prueba pasa los claims reales (rol, usuario, copropiedad), como la prueba SQL 70              |
