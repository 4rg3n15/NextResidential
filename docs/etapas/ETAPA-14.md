...
| **D-124** | `coherencia-estado-etapas.mjs` pasa de 24 a 25 bloques sin ejecutar **sin que su código cambie** —el `git diff` contra `develop` de ese fichero está vacío—. Medido: en una ejecución directa deja los MISMOS 29 bloques con el documento viejo y con el nuevo, así que el delta aparece solo bajo la suite negativa, donde la sonda 14 muta `ESTADO_ETAPAS.md` y una de sus escenas ya no alcanza su `mal()`. Trinquete actualizado con este motivo; falta identificar el bloque y volver a ejercerlo | ETAPA 16    |
